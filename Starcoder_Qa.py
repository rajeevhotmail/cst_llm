...
starcoder_pipeline = pipeline("text-generation", model=model, tokenizer=tokenizer)
...
def get_starcoder_response(question, context=None):
    """Generates a response using StarCoder-3B."""
    try:
        prompt = f"### Context:\n{context}\n\n### Question:\n{question}\n\n### Answer:"
        response = starcoder_pipeline(prompt, max_length=300, do_sample=True, temperature=0.3)
...
def handle_query():
    try:
        ...
        return jsonify({"question": question, "answer": get_starcoder_response(question, context)})
