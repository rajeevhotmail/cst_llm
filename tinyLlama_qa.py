...
model_name = "TinyLlama/TinyLlama-1.1B"  # Open-source, lightweight model

tokenizer = AutoTokenizer.from_pretrained(model_name)
model = AutoModelForCausalLM.from_pretrained(
    model_name,
    device_map="auto",
    offload_folder="/mnt/data/huggingface_offload"  # Ensure weights are stored on disk
)
...
def get_tinyllama_response(question, context=None):
    """Generates a response using TinyLlama-1.1B."""
    try:
        prompt = f"### Context:\n{context}\n\n### Question:\n{question}\n\n### Answer:"
        response = pipeline_model(prompt, max_length=300, do_sample=True, temperature=0.3)
        return response[0]["generated_text"].strip()
    except Exception as e:
        print("❌ TinyLlama error:", str(e))
...
def handle_query():
    try:
        ...
        return jsonify({"question": question, "answer": get_tinyllama_response(question, context)})
