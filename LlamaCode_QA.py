...
FAISS_DB_DIR = os.path.join(BASE_DIR, "faiss_db_llama")
...
model_name = "codellama/CodeLlama-3b-Instruct-hf"  # Smaller model
...
codellama_pipeline = pipeline("text-generation", model=model, tokenizer=tokenizer)
# Load CodeLlama Model
#codellama_pipeline = pipeline("text-generation", model="codellama/CodeLlama-7b-Instruct-hf", device_map="auto")
...
def get_codellama_response(question, context=None):
    """Generates a response using CodeLlama-3B."""
    try:
        prompt = f"### Context:\n{context}\n\n### Question:\n{question}\n\n### Answer:"
        response = codellama_pipeline(prompt, max_length=300, do_sample=True, temperature=0.3)
        return response[0]["generated_text"].strip()
    except Exception as e:
        print("❌ CodeLlama error:", str(e))
...
def handle_query():
    try:
        ...
        return jsonify({"question": question, "answer": get_codellama_response(question, context)})
