...
def handle_query_with_bolt(question, context=None):
    ...
    try:
        ...
        messages = []
        if context:
            # Send context as part of the user's message
            messages.append({
                "role": "user",
                "content": f"{context}\n\nGiven this context, please {question}"
            })
        else:
            messages.append({
                "role": "user",
                ...
                try:
                    for line in response.iter_lines():
                        if line:
                            ...
                            print(f"📥 Received chunk: {line[:100]}...")  # Print first 100 chars
                            ...
                            if line.startswith('0:"') and line.endswith('"'):
                                ...
                                print(f"⚠️ Unexpected chunk format: {line}")
                                # Still append in case format changes
