...
# Initialize the model (do this at app startup)
LLAMA_MODEL_PATH = "/home/paperspace/rajeev/workspace/llama_model/llama-2-7b.Q4_K_M.gguf"
llm = Llama(
...
# Replace the get_openai_response function
def get_llama_response(question, context=None):
    try:
        # Format the prompt similarly to how you did for OpenAI
        system_prompt = (
            ...
            # You can include the rest of your system prompt here
        )
...
"""
        ...
        response = llm(
            ...
            max_tokens=1024,
            temperature=0.3,
            stop=["</s>"]  # Llama 2 chat usually ends responses with </s>
        )
        ...
        import traceback
        print("❌ Llama inference failed:", traceback.format_exc())
...
FAISS_DB_DIR = os.path.join(BASE_DIR, "faiss_bert_improved_db")
                ...
                try:
                    with open(filepath, "r", encoding="utf-8") as file:
                        ...
                        for item in extracted_items:
                            ...
                            text_embedding = get_codebert_embedding(code)
                            ...
                            cst_embedding = get_codebert_embedding(cst_text)
...
def retrieve_code_snippets(query, top_k=20):  # Increased from 15 to 20
    ...
    query_embedding = get_codebert_embedding(query)
...
def handle_query():
    try:
        ...
        response = get_llama_response(question, context)

        # You might still want to filter the response, or you might skip this step for testing
...
def compare_embeddings():
    try:
        ...
        if not query1 or not query2:
            return jsonify({"error": "Two queries are required for comparison"}), 400

        print(f"📊 Comparing embeddings: '{query1}' vs '{query2}'")

        # Generate embeddings using your model (either the original or CodeBERT)
        if 'get_codebert_embedding' in globals():
            # If you've implemented CodeBERT
            embedding1 = get_codebert_embedding(query1)
            embedding2 = get_codebert_embedding(query2)
        else:
            # Using your original embedding model
            embedding1 = embedding_model.encode(query1, convert_to_numpy=True)
            embedding2 = embedding_model.encode(query2, convert_to_numpy=True)

        # Calculate similarity (cosine similarity or euclidean distance)
        from numpy import dot
        from numpy.linalg import norm

        # Flatten if needed (depends on your embedding function's output)
        if embedding1.ndim > 1 and embedding1.shape[0] == 1:
            embedding1 = embedding1.flatten()
        if embedding2.ndim > 1 and embedding2.shape[0] == 1:
            embedding2 = embedding2.flatten()

        # Cosine similarity: dot product divided by magnitudes
        cosine_similarity = dot(embedding1, embedding2) / (norm(embedding1) * norm(embedding2))

        # Euclidean distance
        euclidean_distance = norm(embedding1 - embedding2)

        return jsonify({
            "query1": query1,
            "query2": query2,
            "cosine_similarity": float(cosine_similarity),  # Convert to float for JSON serialization
            "euclidean_distance": float(euclidean_distance),
            "embedding_dimension": embedding1.shape[0]
        })
