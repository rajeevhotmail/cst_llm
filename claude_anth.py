...
def retrieve_project_summary():
    ...
    # Start with a synthesized summary based on file analysis
    if code_snippets:
        all_content = '\n'.join([content for _, content in code_snippets])
        if 'smsc' in all_content.lower():
            overview = """This appears to be a Python library for SMS (Short Message Service) handling via SMSC (Short Message Service Center). 
            
Key components:
- SMS validation (phone numbers, length checks)
- Error handling for long SMS and priority issues
- Unit testing framework for SMS functionality
- Utility functions for SMS parsing and limitations"""
            print("📱 Generated project overview from code analysis")
            return overview + "\n\n--- File Details ---\n"
    if readme_text:
...
def query_claude(question):
    """Queries Claude API with enhanced context and analysis."""

    # Detect question type
    question_lower = question.lower()

    if any(kw in question_lower for kw in ["test", "testing", "unittest"]):
        # Testing related questions - gather all test files
        test_files = [(f, c) for f, c in code_snippets if "test_" in f.lower()]
        if test_files:
            snippets = "\n\n".join([f"File: {f}\n```python\n{c}\n```" for f, c in test_files[:2]])
            prompt = f"""This is a Python project related to SMS handling. Analyze these test files to answer the question about testing:

{snippets}

Question: {question}
"""
    elif any(kw in question_lower for kw in ["error", "exception", "handling"]):
        # Error handling questions - look for exception definitions
        error_files = [(f, c) for f, c in code_snippets if "error" in c.lower() or "exception" in c.lower()]
        if error_files:
            snippets = "\n\n".join([f"File: {f}\n```python\n{c}\n```" for f, c in error_files[:2]])
            prompt = f"""This is a Python project related to SMS handling. Analyze these error handling implementations:
...
"""
        ...
        if result is None or result[0] is None:
            return "I couldn't find relevant code snippets for your question. Could you try rephrasing it or ask about a specific aspect of the SMS handling system?"

        filename, snippet = result
        extracted_cst = extract_cst_features(snippet)

        prompt = f"""This is a Python SMS handling library. Analyze this code snippet from `{filename}`:

```python
{snippet}
```

Key structural elements (CST):
```text
{extracted_cst}
```
...
"""
    ...
    try:
        print("Making API call with model: claude-3-opus-20240229")
...
def handle_query():
    ...
    project_keywords = ["project overview", "what is the project about", "workflow", "summarize the project", "what does this project do", "project summary"]
